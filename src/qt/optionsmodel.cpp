...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeaerumAmount"))
        settings.setValue("nAnonymizeaerumAmount", 1000);
    nAnonymizeaerumAmount = settings.value("nAnonymizeaerumAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeaerumAmount"))
        SoftSetArg("-anonymizeaerumamount", settings.value("nAnonymizeaerumAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeaerumAmount:
            return QVariant(nAnonymizeaerumAmount);
        default:
            return QVariant();
        }
    }
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeaerumAmount:
            nAnonymizeaerumAmount = value.toInt();
            settings.setValue("nAnonymizeaerumAmount", nAnonymizeaerumAmount);
            emit AnonymizeaerumAmountChanged(nAnonymizeaerumAmount);
            break;
        default:
            break;
        }

...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeaerumAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeaerumAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeaerumAmount = strAnonymizeaerumAmount.remove(strAnonymizeaerumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeaerumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeaerumAmount*COIN) nMaxToAnonymize = nAnonymizeaerumAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeaerumAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeaerumAmount));
        strAnonymizeaerumAmount = strAnonymizeaerumAmount.remove(strAnonymizeaerumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeaerumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
